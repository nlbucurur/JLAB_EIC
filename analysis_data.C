...
void analysis_data()
{
  ...
  for (const auto &[var, range] : branch_names)
  {
    ...
    h_base_data->SetMaximum(1.4 * h_base_data->GetMaximum());
  ...
  for (const auto &[label_cut, cut] : cuts)
  {
    ...
    TString filename = Form("./cuts_data/optimization_%s_data.png", label_cut.Data());
    TString filename_pdf = Form("./cuts_data/optimization_%s_data.pdf", label_cut.Data());
