...
int TMVAClassificationNew( TString myMethodList = "" )
{
   ...
   if (!gSystem->AccessPathName( fname_s ) && !gSystem->AccessPathName( fname_b )) {
      ...
      input_s = TFile::Open("http://root.cern/files/tmva_class_example.root", "CACHEREAD");
   }
   if (!input_s || !input_b) {
...
//    dataloader->AddVariable( "var4",                "Variable 4", "units", 'F' );
   ...
   dataloader->AddVariable( "_mm2_eNg", "mm2_eNg missing Neutron", "GeV", 'F' );
   dataloader->AddVariable( "_mm2_eNg_N", "mm2_eNg_N All detected", "GeV", 'F' );
   dataloader->AddVariable( "_mm2_eNX_N", "mm2_eNX_N missing photon", "GeV", 'F' );
   dataloader->AddVariable( "_mm2_eg", "mm2_eg missing proton", "GeV", 'F' );
