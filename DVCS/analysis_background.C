...
std::vector<std::pair<TString, TCut>> generate_cuts(const std::map<TString, TH1D *> &hs_base)
{
  std::vector<std::pair<TString, TString>> cuts_definitions = {
      ...
      {"_chi2pid", "_strip_El_chi2pid >= -4.37898 && _strip_El_chi2pid <= 3.77798 && _strip_Nuc_chi2pid >= -5.57697 && _strip_Nuc_chi2pid <= 6.33990"},
      {"_delta_t", "_delta_t >= -1.03182 && _delta_t <= 1.27318"},
      {"_delta_phi", "abs(fmod(_delta_Phi, 180)) <= 1.5"},
      {"_mm2_eNg_neutron_expected", "_mm2_eNg >= -1.22968 && _mm2_eNg <= 3.67237"},
      {"_mm2_eNg_N_nothing_expected", "_mm2_eNg_N >= -0.55184 && _mm2_eNg_N <= 0.47227"},
      {"_mm2_eNX_N_photon_expected", "_mm2_eNX_N >= -4.09789 && _mm2_eNX_N <= 4.17903"},
      {"_mm2_eg_proton_expected", "_mm2_eg >= -1.35581 && _mm2_eg <= 3.93882"}};
...
void analysis_background()
{
  ...
  tree->SetBranchAddress("_strip_El_px", &strip_El_px);
  tree->SetBranchAddress("_strip_El_py", &strip_El_py);
  tree->SetBranchAddress("_strip_El_pz", &strip_El_pz);
  tree->SetBranchAddress("_strip_El_E", &strip_El_E);
  tree->SetBranchAddress("_strip_Nuc_px", &strip_Nuc_px);
  tree->SetBranchAddress("_strip_Nuc_py", &strip_Nuc_py);
  tree->SetBranchAddress("_strip_Nuc_pz", &strip_Nuc_pz);
  tree->SetBranchAddress("_strip_Nuc_E", &strip_Nuc_E);
  tree->SetBranchAddress("_strip_Ph_px", &strip_Ph_px);
  tree->SetBranchAddress("_strip_Ph_py", &strip_Ph_py);
  tree->SetBranchAddress("_strip_Ph_pz", &strip_Ph_pz);
  tree->SetBranchAddress("_strip_Ph_E", &strip_Ph_E);
  ...
  for (int i = 0; i < tree->GetEntries(); i++)
  {
    ...
    Ph_Vec.SetPxPyPzE(strip_Ph_px, strip_Ph_py, strip_Ph_pz, strip_Ph_E);
    Nuc_Vec.SetPxPyPzE(strip_Nuc_px, strip_Nuc_py, strip_Nuc_pz, strip_Nuc_E);
    El_Vec.SetPxPyPzE(strip_El_px, strip_El_py, strip_El_pz, strip_El_E);
